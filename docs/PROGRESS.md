...
| 2026-09-09 | Migrated Stripe → PayMongo QR Ph (dynamic per-transaction QR, 1.34%): new `lib/paymongo.ts` (intent+attach, HMAC webhook verify, refund), create-intent returns QR image, webhook handles payment.paid/failed + qrph.expired, refund resolves pay_* id. Unified QrPay UI: PayMongo QR primary with booking-status polling, manual reference-QR fallback when keys unset (keeps keyless CI green). Removed stripe dep + CSP entries. Typecheck/lint clean, 20 unit + 3 e2e green. Needs PAYMONGO_* keys for live money. |
